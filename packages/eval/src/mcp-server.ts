#!/usr/bin/env node
/**
 * The world of tasks.ts served over MCP in one of the eval layouts, so an external agent
 * (Claude Code in headless mode) can be measured instead of a hand-written loop.
 *
 *   ONETOOL_EVAL_LAYOUT=onetool         four generic tools with bare descriptions
...
import { isPlainObject, toStructured, type CallContext, type Content, type ToolSpec } from "@o6lvl4/onetool-core";
import { buildWorld, type EvalLayout } from "./tasks.js";

export type Layout = EvalLayout;
...
const layout: Layout = LAYOUTS.find((l) => l === process.env["ONETOOL_EVAL_LAYOUT"]) ?? "onetool-inline";
...
function toMcpTool(s: ToolSpec): Tool {
  return {
    name: s.name,
...
async function main(): Promise<void> {
  const { onetool } = buildWorld({ layout, padding });
